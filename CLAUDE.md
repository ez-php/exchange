...
Currency exchange-rate lookup and conversion for `ez-php/money` values. `Money` deliberately has no `convertTo()` (see `modules/money/CLAUDE.md`) — conversion needs a rate source, which is an external concern this package supplies.
...
### ExchangeRateProviderInterface (`src/ExchangeRateProviderInterface.php`)

Single method: `getRate(Currency|string $base, Currency|string $quote): BigDecimal`, where a rate of R means `1 base = R quote`. Implementations throw `ExchangeRateNotFoundException` when no rate is available.

### StaticExchangeRateProvider (`src/StaticExchangeRateProvider.php`)

In-memory driver backed by a nested `[baseCode => [quoteCode => rate]]` table, settable at construction or via `setRate()`. Same-currency pairs always return `1` without a table lookup. This is the standard driver for tests and for applications that maintain their own (e.g. periodically refreshed) rate table.

### HttpExchangeRateProvider (`src/HttpExchangeRateProvider.php`)
...
- No external infrastructure required (no database, no Redis, no real network calls)
- `HttpExchangeRateProviderTest` uses `ez-php/http-client`'s `FakeTransport` directly (constructing `HttpClient(new FakeTransport([...]))`), never the `Http` static façade, so no global state needs resetting between tests
- Test classes live in the shared `Tests\` namespace but must be uniquely named
  ...
  `Exchange` when the obvious name is already taken.
